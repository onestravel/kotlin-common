 ...
 * @version 1.0.0
 */
abstract class OneObserver1<DATA : Serializable> : Observer<ResponseResult1<DATA>>,ObserverResult<ResponseResult1<DATA>> {
    ...
    override fun onNext(result: ResponseResult1<DATA>) {
        if (OneResponse.REQUEST_OK == result.code) {
            onSuccess(result)
